...
| `discovery/discovery_environment.txt`, `discovery_file_system.txt` | STXT-DISCOVERY-SPEC §§4, 6 | `src/discovery/DiscoveryEnvironment.ts`, `DiscoveryFileSystem.ts` | `dev.stxt.discovery.DiscoveryEnvironment` (+ `SystemDiscoveryEnvironment`), `DiscoveryFileSystem` (+ `NioDiscoveryFileSystem`, since 1.0) |
...
`ConditionalValidator` wrapper the ports used to ship was removed in 1.0: since 0.8.0 the
